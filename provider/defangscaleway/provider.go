...
const Name string = "defang-scaleway"
...
func Provider() p.Provider {
	return infer.Provider(infer.Options{
		...
		Components: []infer.InferredComponent{
			...
			infer.Component[*Postgres, ScalewayPostgresInputs, *ScalewayPostgresOutputs](&Postgres{}),
			infer.Component[*Redis, ScalewayRedisInputs, *ScalewayRedisOutputs](&Redis{}),
		},
		ModuleMap: map[tokens.ModuleName]tokens.ModuleName{
			"provider":       "index",
			"defangscaleway": "index",
		},
		Metadata: schema.Metadata{
			Description: "Deploy containerized services to Scaleway with Pulumi.",
			Keywords: []string{
				"category/cloud", "category/infrastructure", "kind/native", "defang", "docker",
				"cloud", "scaleway", "containers",
			},
			...
			LanguageMap: map[string]any{
				...
				"nodejs": nodejsGen.NodePackageInfo{
					PackageName: "@defang-io/pulumi-defang-scaleway",
				},
