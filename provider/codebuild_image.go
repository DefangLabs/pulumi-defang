...
// Matches the TS CodeBuildImageBuild dynamic resource: starts build, polls until done, returns image URL.
// If Create fails, the resource is not recorded in state → next `pulumi up` retries.
...
// Matches TS codeBuildImageBuildProvider.create exactly.
func (*CodeBuildImageBuild) Create(ctx context.Context, req infer.CreateRequest[CodeBuildImageBuildInputs]) (infer.CreateResponse[CodeBuildImageBuildState], error) {
	...
	// Initial wait for IAM role to sync (matches TS: await setTimeout(3000))
	...
	// For now, return the destination as the image (digest resolution can be added later)
...
// isRetryable determines if a build error can be retried.
// Matches TS isRetryable: do not retry on explicit aborts or docker buildx errors.
...
// runCodeBuildBuild starts a CodeBuild build and polls until completion.
// Matches TS runCodeBuildBuild.
func runCodeBuildBuild(ctx context.Context, projectName, region string, maxWaitSeconds int) (string, error) {
	...
	// Poll until done (matches TS waitUntilBuildDone with minDelay=2s, maxDelay=5s)
