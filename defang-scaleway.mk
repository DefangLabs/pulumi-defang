PACK             := defang-scaleway
PACKDIR          := sdk
PROJECT          := github.com/DefangLabs/pulumi-defang
NODE_MODULE_NAME := @defang-io/pulumi-defang-scaleway
NUGET_PKG_NAME   := DefangLabs.defang-scaleway
...
VERSION_PATH    := ${PROVIDER_PATH}/defangscaleway.Version
...
.sdk.go.$(PACK).stamp: $(WORKING_DIR)/bin/$(PROVIDER)
	...
	mkdir -p "$(dir $(SDK_GO_DIR))" && mv "$(WORKING_DIR)/.sdk.tmp.$(PACK)/go/defangscaleway" "$(SDK_GO_DIR)" && rm -rf "$(WORKING_DIR)/.sdk.tmp.$(PACK)"
